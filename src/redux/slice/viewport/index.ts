...
export const screenToWorld = (screen: Point, translate: Point, scale: number): Point => ({
  x: (screen.x - translate.x) / scale,
  y: (screen.y - translate.y) / scale,
});

export const worldToScreen = (world: Point, translate: Point, scale: number): Point => ({
...
export const zoomAroundScreenPoint = (
  ...
  const worldAtOrigin = screenToWorld(originScreen, currentTranslate, currentScale);
  return {
...
const viewportSlice = createSlice({
  ...
  reducers: {
    ...
    setScale(state, action: PayloadAction<{ scale: number; originScreen?: Point }>) {
    ...
    // Relative zoom
    zoomBy(state, action: PayloadAction<{ factor: number; originScreen: Point }>) {
      const { factor, originScreen } = action.payload;
      const next = clamp(state.scale * factor, state.minScale, state.maxScale);
      const t = zoomAroundScreenPoint(originScreen, next, state.translate, state.scale);
      state.translate.x = t.x;
      state.translate.y = t.y;
      state.scale = next;
    },
    ...
    panStart(state, action: PayloadAction<{ screen: Point; mode?: ViewportMode }>) {
    ...
    centerOnWorld(state, action: PayloadAction<{ world: Point; toScreen?: Point }>) {
    ...
    zoomToFit(
      ...
      const scaleX = aw / bw;
      const scaleY = ah / bh;
      const next = clamp(Math.min(scaleX, scaleY), state.minScale, state.maxScale);
    ...
    restoreViewport(state, action: PayloadAction<{ scale: number; translate: Point }>) {
